...
$(function(){
	...
	if (errorinfo.val()==null || errorinfo.val() == "") {
		a_orgCodePicPath.next().html("*上传大小不能超过500KB*上传文件类型必须为:jpg,jpg,png,pneg");
	}else{
		a_orgCodePicPath.next().html(errorinfo.val());
	}
});

$(function(){
	a_companyLicPicPath=$("#a_companyLicPicPath");
	errorinfo_wp = $("#errorinfo-wp");
	if (errorinfo_wp.val()==null || errorinfo_wp.val() == "") {
		a_companyLicPicPath.next().html("*上传大小不能超过500KB*上传文件类型必须为:jpg,jpg,png,pneg");
	}else{
		a_companyLicPicPath.next().html(errorinfo_wp.val());
	}
});
$(function(){
	proCode = $("#proCode");
